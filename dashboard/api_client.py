...
def parse_prediction(result: dict) -> dict:
    ...
    tool_prediction = result.get("tool_prediction") or {}
    maintenance_prediction = result.get("maintenance_prediction") or {}
    ...
    if result.get("source") in {"service", "api", "ai4i_only", "tool_wear_only"}:
        machine_health = float(maintenance_prediction.get("machine_health", result.get("machine_health", 0.0)) or 0.0)
        tool_health_val = tool_prediction.get("tool_health", result.get("tool_health"))
        ...
        failure_prob = _pct(
            maintenance_prediction.get("failure_probability", result.get("failure_probability", result.get("failure_prob", 0.0)))
        )
        overall_risk = float(decision.get("overall_risk", result.get("overall_risk", failure_prob or 0.0)))
        machine_status = decision.get("overall_status", result.get("overall_machine_status", result.get("machine_status", "Unknown")))

        return {
            "vb": float(tool_prediction.get("tool_wear", result.get("vb", 0.0))),
            "rul": float(tool_prediction.get("remaining_useful_life", result.get("rul", 0.0))),
            "tool_health": round(tool_health, 1),
            "failure_risk": int(round(overall_risk)),
            "machine_status": machine_status,
            "wear_level": tool_prediction.get("wear_level", result.get("wear_level", "")),
            "action": tool_prediction.get("maintenance_action", result.get("action", "")),
            "confidence": result.get("confidence", ""),
            "next_inspection": result.get("next_inspection", ""),
            "wear_limit": float(result.get("wear_limit", 0.3)),
            "failure_probability": failure_prob,
            "machine_failure": maintenance_prediction.get("machine_failure", result.get("machine_failure", "No")),
            "failure_type": maintenance_prediction.get("failure_type", result.get("failure_type", "")),
            "failure_type_confidence": _pct(result.get("failure_type_confidence", result.get("fail_confidence", 0.0))),
            "machine_health": machine_health,
            "severity_level": maintenance_prediction.get("severity", result.get("severity_level", result.get("severity", ""))),
            "components_to_inspect": _parse_list(result.get("components_to_inspect", result.get("components", []))),
            "recommended_actions": _parse_list(recommendation.get("operator_actions", result.get("recommended_actions", result.get("recommendations", [])))),
            ...
            "operator_summary": recommendation.get("operator_summary", result.get("operator_summary", result.get("recommendations", ""))),
            ...
            "maintenance_prediction": maintenance_prediction if maintenance_prediction else None,
            "decision": decision,
            "recommendation": recommendation,
            "metadata": metadata,
            "risk_breakdown": decision.get("risk_breakdown", {}),
            "should_replace_tool": recommendation.get("should_replace_tool"),
            "should_inspect_spindle": recommendation.get("should_inspect_spindle"),
            "recommended_components": recommendation.get("recommended_components", []),
            "maintenance_schedule": recommendation.get("maintenance_schedule", ""),
        }
