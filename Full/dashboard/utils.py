    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "steps": [
                ...
                {"range": [max_val * 0.5, max_val * 0.75], "color": "#fef9c3"},
...
def wear_line_chart(cycles=None, actual=None, predicted=None, threshold=0.8):
    if cycles is None:
        cycles = np.arange(0, 101, 5)
        actual = np.clip(0.005 * cycles + np.random.normal(0, 0.01, len(cycles)), 0, 1)
        predicted = 0.005 * cycles + 0.02
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=True, gridcolor="#f1f5f9", range=[0, 1.05]),
    )
    return fig


def rul_line_chart(rul_val=12.5, threshold=0.8):
    cycles = np.arange(0, 121, 5)
    wear   = np.clip(0.0065 * cycles, 0, 1)
    rul_cycle = int(rul_val / 0.0065) if rul_val else 75
    ...
    fig.add_vrect(x0=rul_cycle, x1=cycles[-1], fillcolor="#fee2e2", opacity=0.25, line_width=0,
...
def shap_bars(features: dict):
    """Render inline HTML SHAP bars using only inline styles (no CSS classes)."""
    ...
    for feat, val in features.items():
        ...
        rows += (
            ...
            f'<div style="width:{pct}%;background:#3b82f6;border-radius:4px;height:10px"></div></div>'
...
def risk_badge(risk_pct):
    if risk_pct < 30:
        ...
        bg, col, txt = "#fef9c3", "#ca8a04", "Medium Risk"
    else:
